...
namespace ManualSaveMod
{
    internal static class ModLocalization
    {
        ...
        internal static void Initialize(string modFolder)
        {
            ...
            if (!File.Exists(path))
            {
                Debug.Log($"[ManualSaveMod] {DataFileName} not found at {path}, using default labels");
                return;
            }
            ...
            {
                ParseIni(path);
                Debug.Log($"[ManualSaveMod] Loaded localization from {path}");
            }
            catch (Exception ex)
            {
                Debug.LogError($"[ManualSaveMod] Failed to read {DataFileName}: {ex}");
            }
        }
        ...
        private static void ParseIni(string path)
        {
            ...
            foreach (string rawLine in File.ReadAllLines(path))
            {
                ...
                {
                    Debug.LogWarning($"[ManualSaveMod] Unknown language token '{langToken}' in {DataFileName}");
                }
            }
        }
